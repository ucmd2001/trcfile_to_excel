...
for i in filtered_df.index:
    if filtered_df.loc[i, 'previous_value'] == '11' and filtered_df.loc[i, 10] == '01':
        target_value = filtered_df.loc[i, 3] # 第 3 列的值，例如 '0481'
        break
...
if 'target_value' in locals():
    ...
    # 計算提取範圍
    start_pos = max(position - 25, 0)
    end_pos = min(position + 25, len(final_filtered_df) - 1)

    # 提取所需行
    selected_rows = final_filtered_df.index[start_pos:end_pos + 1]
